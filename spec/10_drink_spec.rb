...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:sake) { described_class.new('sake') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(sake.type).to eql('sake')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(sake.ounces).to eql(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:aguardiente) { described_class.new('aguardiente')}

      # remove the 'x' before running this test
      it 'is full' do
        expect(aguardiente).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:mezcal) { described_class.new('mezcal', 3)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(mezcal).not_to be_full
      end
    end
  end
end
