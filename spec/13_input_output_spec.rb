...
describe NumberGame do
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        invalid_entry_first = 'aa'
        invalid_entry_second = '12'
        valid_entry = '7'
        allow(game_loop).to receive(:player_input).and_return(\
          invalid_entry_first, invalid_entry_second, valid_entry
        )
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      ...
      subject(:long_game) { described_class.new(5, '5', 5)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        long_game_phrase = "That was hard. It took you 5 guesses!\n"
        expect { long_game.final_message }.to output(long_game_phrase).to_stdout
      end
    end
  end
end
